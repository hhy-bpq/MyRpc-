...
//        ChannelBuffer buffer = new NettyBackedChannelBuffer(out);
     ...
     * @throws Exception
     */
    private void encodeRequest(ByteBuf  out,Request req) throws Exception {
        // header.
     ...
     * @throws Exception
     */
    private void encodeResponse( ByteBuf out,Response res) throws Exception {
        // header.
        byte[] header = new byte[HEADER_LENGTH];
        ...
        if(StringUtils.isBlank(res.getErrorMsg())){//errormsg 是空 表示status 是成功 的 20表示成功  参照dubbo
            header[3]=20;
        }
        ...
        try {
            ...
            //Hessian的序列化输出
            ho = new HessianOutput(os);
            if(StringUtils.isBlank(res.getErrorMsg())){
                ho.writeObject(res.getResult());
            }else{
                ho.writeString(res.getErrorMsg());
            }
            body = os.toByteArray();
        } catch (IOException e) {
            throw e;
